...
> **Decision to validate early (spike in Phase 0).** This single-worker re-export is the primary approach (one deploy, same-origin WS). If the build/bundling proves brittle with the adapter version, fall back to a **separate Durable Object Worker** bound to the SvelteKit worker; the client then opens the WS against that worker's URL (needs CORS + an env-configured WS origin). Keep `protocol.ts` and `rules.ts` transport-agnostic so either layout works unchanged. See [03](./03-realtime-and-game-logic.md).
...
## Bindings (wrangler.jsonc target)

Phase 0 adds these (currently none are declared — `hooks.server.ts` already references `platform.env.DB`, so auth throws until this lands):
...
```jsonc
{
  ...
  "d1_databases": [
    { "binding": "DB", "database_name": "footbol-db", "database_id": "<id>", "migrations_dir": "drizzle" }
  ],
  "r2_buckets": [
    { "binding": "ASSETS_BUCKET", "bucket_name": "footbol-assets" }
  ],
  "kv_namespaces": [
    { "binding": "POOL_CACHE", "id": "<id>" }
  ],
  "durable_objects": {
...
Run `pnpm gen` (`wrangler types`) after editing this to regenerate the global `Env` type; `pnpm check`/`build` depend on it.
