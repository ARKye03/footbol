...
2. **`src/lib/server/durable/game-room.ts`** — the `GameRoom` Durable Object. Owns sockets + storage; _delegates all game decisions to `rules.ts`_; broadcasts results.
...
- **Authority**: clients send _intents_; the DO is the only writer. Clients never trust their own optimistic state for anything adjudicated (turns, guesses).
