...
**The app must be fully playable on your laptop, including realtime.** The tension: `vite dev` gives fast HMR but doesn't run our custom `src/worker.ts` (so no `/ws/*` Durable Object routing); `wrangler dev` runs the real Workers runtime (DO/WS/D1/R2) but on the *built* worker. So we use **two modes**.
