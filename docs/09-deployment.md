...
Fill the ids in `wrangler.jsonc` ([01](./01-architecture.md)). The DO needs a `migrations` entry (`new_sqlite_classes: ["GameRoom"]`) — that's a *Workers* migration (registers the class), separate from D1 migrations.
